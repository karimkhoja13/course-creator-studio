  ...
  return (
    <main className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="h-12 flex items-center gap-4 px-6 border-b border-[rgba(255,255,255,0.05)]">
        ...
        <span className="text-fluid-text-muted text-sm">4 Chapters</span>
        <span className="text-fluid-text-muted text-sm">Total Duration: 12m 30s</span>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="flex items-center gap-4 mb-4 ml-8">
          <span className="text-fluid-text-muted text-sm font-medium">1</span>
          <div className="flex-1 flex items-center gap-3 py-3 px-4 rounded-lg bg-[rgba(255,255,255,0.02)] hover:bg-[rgba(255,255,255,0.04)] transition-colors cursor-pointer">
            <span className="w-2 h-2 rounded-full bg-fluid-cyan pulse-glow-slow" />
            <span className="text-fluid-text-primary">Introduction to Variables</span>
            <span className="text-fluid-text-muted text-sm ml-auto">3:00</span>
        ...
        {/* Chapter 2 - Active/Expanded */}
        <div className="flex gap-4 mb-4 ml-8">
          <span className="text-fluid-text-muted text-sm font-medium pt-4">2</span>
          <div className="flex-1 rounded-xl bg-[rgba(255,255,255,0.02)] shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_4px_24px_rgba(0,0,0,0.2)] overflow-hidden">
            {/* Chapter Header */}
            <div className="p-4 flex items-center gap-3">
              <span className="px-2 py-1 rounded text-xs font-semibold bg-[rgba(45,212,191,0.15)] text-fluid-cyan">
                ACTIVE CHAPTER
              </span>
              <span className="px-2 py-1 rounded text-xs font-semibold bg-[rgba(255,255,255,0.08)] text-fluid-text-muted">
                DRAFT
              </span>
              <div className="ml-auto flex items-center gap-2">
                <div className="w-7 h-7 rounded-full bg-gradient-to-br from-cyan-400 to-purple-500 flex items-center justify-center text-xs">
                  C
                </div>
                <div className="flex flex-col">
                  <span className="text-xs text-fluid-text-muted">TONE</span>
            ...
            <div className="px-4 pb-4">
              <h2 className="text-2xl font-semibold mb-3">Chapter 2: Understanding Agents</h2>
              <div className="flex items-center gap-2 text-fluid-text-muted text-sm">
                <Icons.Target />
                <span>
                  Objective: Teach students how to initialize an Agent class and...
                </span>
              </div>
            </div>
            ...
            <div className="px-4 py-6 border-t border-[rgba(255,255,255,0.05)]">
              ...
              {/* Narrative Block 1 */}
              <div className="mb-6">
                <p className="text-fluid-text-primary leading-relaxed mb-4">
                  Let's start by explaining the{' '}
                  <code className="px-2 py-0.5 rounded bg-fluid-bg-elevated font-mono text-fluid-purple text-sm">
                    __init__
                  </code>{' '}
                  function. Think of it like starting a car engine—without this spark, the agent is
                  just a lifeless shell of code. We need to pass in memory parameters immediately.
                </p>

                {/* Code Block */}
                <div className="rounded-lg bg-fluid-bg-deep overflow-hidden shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_4px_16px_rgba(0,0,0,0.3)]">
                  <div className="flex items-center justify-between px-4 py-2 bg-[rgba(255,255,255,0.03)]">
                    <div className="flex items-center gap-2">
                      <Icons.Code />
                      <span className="text-sm text-fluid-text-muted">agent.py</span>
                      <span className="text-xs text-fluid-text-muted">(lines 10-14)</span>
                      <span className="text-xs text-fluid-purple">5 lines</span>
                    </div>
                    <Icons.ChevronDown />
                  </div>
                  <pre className="p-4 font-mono text-sm overflow-x-auto">
                    <code>
                      ...
                      <span className="code-parameter">memory_size</span>=
                      <span className="code-number">1024</span>):
                      {'\n'}
                      {'        '}
                      <span className="code-comment"># Initialize the memory buffer</span>
                      {'\n'}
                      {'        '}
                      <span className="code-parameter">self</span>.memory = [
                      <span className="code-number">0</span>] *{' '}
                      <span className="code-variable">memory_size</span>
                      {'\n'}
                      {'        '}
                      <span className="code-parameter">self</span>.state ={' '}
                      <span className="code-string">"IDLE"</span>
                    </code>
                  </pre>
                </div>

                {/* PDF Reference */}
                <div className="flex items-center gap-2 mt-4 px-3 py-2 rounded-lg bg-[rgba(255,255,255,0.03)] w-fit">
                  <span className="text-red-400">
                    <Icons.Pdf />
                  </span>
                  <span className="text-sm text-fluid-text-muted">Research_Paper.pdf</span>
                  <span className="text-xs text-fluid-text-muted">(p. 4)</span>
                </div>
              </div>
              ...
              <div className="mb-6">
                <p className="text-fluid-text-primary leading-relaxed mb-4">
                  ...
                  logic. This is where the agent decides whether to act or remain idle based on its
                  current energy level.
                </p>

                {/* Collapsed Code Block */}
                <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[rgba(255,255,255,0.03)] w-fit cursor-pointer hover:bg-[rgba(255,255,255,0.05)] transition-colors">
                  <Icons.Code />
                  <span className="text-sm text-fluid-text-muted">agent.py (lines 20-35)</span>
                  <span className="text-fluid-text-muted">
                    <Icons.Eye />
                  </span>
                </div>
              ...
              {/* Add Narrative Block Button */}
              <button className="w-full py-3 border-2 border-dashed border-[rgba(255,255,255,0.1)] rounded-lg text-fluid-text-muted hover:border-[rgba(255,255,255,0.2)] hover:text-fluid-text-primary transition-all flex items-center justify-center gap-2">
                <Icons.Plus />
                Add Narrative Block
              </button>
            </div>
